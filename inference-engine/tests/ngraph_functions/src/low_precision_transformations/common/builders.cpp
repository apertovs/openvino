...
std::shared_ptr<Node> makeDequantization(
    ...
    if (!dequantizationOperations.convert.empty()) {
        std::shared_ptr<ngraph::opset1::Convert> convert = ngraph::pass::low_precision::make_dequantization_operation<ngraph::opset1::Convert>(
            data,
    ...
    if (!dequantizationOperations.subtract.empty()) {
        ...
        if ((dequantizationOperations.subtract.outPrecision == element::undefined) ||
            ...
            subtract = ngraph::pass::low_precision::make_dequantization_operation<ngraph::opset1::Subtract>(parent, subtractConst);
        } else {
            subtract = ngraph::pass::low_precision::make_dequantization_operation<op::TypeRelaxed<ngraph::opset1::Subtract>>(parent, subtractConst);
    ...
    if (!dequantizationOperations.multiply.empty()) {
        ...
        std::shared_ptr<ngraph::opset1::Multiply> multiply = ngraph::pass::low_precision::make_dequantization_operation<ngraph::opset1::Multiply>(
            parent,
